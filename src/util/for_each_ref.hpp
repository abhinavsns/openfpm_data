...
template<>
struct for_each_ref_impl<false>
{
    template<
          typename Iterator
        , typename LastIterator
...
template<>
struct for_each_ref_cpu_impl<false>
{
    template<
          typename Iterator
        , typename LastIterator
...
__host__ __device__ inline void for_each_ref(F & f, Sequence* = 0, TransformOp* = 0)
{
    ...
    aux::for_each_ref_impl< boost::is_same<first,last>::value >
...
template<
    ...
    , typename F
    >
inline void for_each_cpu_ref(F & f, Sequence* = 0, TransformOp* = 0)
{
    BOOST_MPL_ASSERT(( is_sequence<Sequence> ));
...
template<
    ...
    >
inline
void for_each_cpu_ref(F & f, Sequence* = 0)
{
    for_each_ref<Sequence, identity<> >(f);
}
